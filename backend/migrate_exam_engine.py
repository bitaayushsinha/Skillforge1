...
def migrate():
    try:
        # Create any missing tables (creates exam_results table)
        Base.metadata.create_all(bind=engine)
        logger.info("Base.metadata.create_all completed successfully.")

        columns_to_add = [
            ("exam_slot_bookings", "link_status", "VARCHAR(255) DEFAULT 'pending'"),
            ("exam_slot_bookings", "exam_engine_session_ref", "VARCHAR(255)"),
        ]
        ...
        logger.info("Exam Engine integration migration completed.")
    except Exception as e:
